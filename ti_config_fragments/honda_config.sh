...
# If the board has rear camera on NTSC
if [ "$TARGET_HAS_NTSC_RWC" == "true" ]
then
   echo "Board has NTSC RVC"
   CONFIG_FRAGMENTS=" \
$CONFIG_FRAGMENTS \
$KERNEL_ROOT/ti_config_fragments/ntsc_rvc.cfg "
else
    echo "Board DOES NOT have NTSC RVC: $TARGET_HAS_NTSC_RWC ."
fi
