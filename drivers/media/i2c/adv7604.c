...
#define ADV7604_ENABLE_COLOR_CONTROLS       (1 << 7)
#define ADV7604_VID_ADJ_EN_REGISTER         (0x3E)

#define BRIGHTNESS_MAX                      (508)
...
static int adv76xx_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret = 0;
	switch (ctrl->id) {
	...
	case V4L2_CID_PRIVATE_HDMI_DEVICE_ID:
		ctrl->value = ADV7611_REVISION_ID;
		break;
	default:
		ret = -EINVAL;
	}
...
{
    int hdmi_width[]  = {640, 720, 800, 1024, 1280, 1920,};
    int hdmi_height[] = {480, 576, 600,  768,  720, 1080,};

    if (fsize->index >= ARRAY_SIZE(hdmi_width))
        return -EINVAL;

    fsize->type = V4L2_FRMSIZE_TYPE_DISCRETE;
    fsize->discrete.width  = hdmi_width[fsize->index];
    fsize->discrete.height = hdmi_height[fsize->index];
        ...
        struct v4l2_mbus_framefmt *fmt)
{
    ...
    state->timings.bt.width  = fmt->width;
    state->timings.bt.height = fmt->height;
...
static const struct v4l2_ctrl_ops adv76xx_ctrl_ops = {
	...
	.g_volatile_ctrl = adv76xx_g_ctrl,
};
...
static int adv76xx_core_init(struct v4l2_subdev *sd)
{
	...
	/* Enable color controls, VID_ADJ_EN, Addr 44 (CP), Address 0x3E[7] */
	cp_write(sd, ADV7604_VID_ADJ_EN_REGISTER, ADV7604_ENABLE_COLOR_CONTROLS);
			 ...
			 const struct i2c_device_id *id)
{
	...
	v4l2_ctrl_new_std(hdl, &adv76xx_ctrl_ops,
			V4L2_CID_BRIGHTNESS, BRIGHTNESS_MIN, BRIGHTNESS_MAX, BRIGHTNESS_STEP,
			BRIGHTNESS_DEFAULT);
	v4l2_ctrl_new_std(hdl, &adv76xx_ctrl_ops,
	...
	v4l2_ctrl_new_std(hdl, &adv76xx_ctrl_ops,
			V4L2_CID_HUE, HUE_MIN, HUE_MAX, HUE_STEP, HUE_DEFAULT);
