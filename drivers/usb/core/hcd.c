...
/* For Honda/Acura.
 * show whether external usb hub is available on this usb controller.
 * Honda requested to enable external hub on specific case.(eg. diagnostic for AndroidAuto)
 */
static ssize_t ext_hubs_show(struct device *dev,
...
/* For Honda/Acura.
 * set for enabling external hub on this usb controller.
 * Honda requested to enable external hub on specific case.(eg. diagnostic for AndroidAuto)
 */
static ssize_t ext_hubs_store(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t count)
{
	...
	if (!!val)
		set_bit(HCD_FLAG_ENABLE_HUBS_ON_EXT_PORTS, &hcd->flags);
	else
		clear_bit(HCD_FLAG_ENABLE_HUBS_ON_EXT_PORTS, &hcd->flags);
	return count;
}
static DEVICE_ATTR_RW(ext_hubs);
#endif
