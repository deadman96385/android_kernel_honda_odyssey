...
/*
 * XXX DEBUG for maximum turnaround time of the TJBA display transactions.
 */
struct i2c_wtr_turnaround i2c_wtr_max_turnaround_0x55;
uint32_t hralvds_i2c_msleep = 0;
			     ...
			     struct i2c_msg *msg, int stop)
{
	...
	if (hralvds_i2c_msleep)
		msleep(hralvds_i2c_msleep);
...
static u64 _0x55_start = 0;
static u64 _0x55_delta_t = 0;
static u64 _0x55_max_t = 0;

struct i2c_wtr_turnaround max_i2c_wtr_turnaround_0x55;

#define i2c_turnaround max_i2c_wtr_turnaround_0x55
...
omap_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	for (i = 0; i < num; i++) {
		/*
		 * for messages directed at the TJBA display panel, measure
		 *	the amount of time between the start of the write
		 *	and read transactions.  save the max time along with
		 *	the kernel current time.
		 */
		if (0x55 == msgs[i].addr && num > 1) {
			if (0 == i) {
				_0x55_start = local_clock();
			} else {
				_0x55_delta_t = local_clock() - _0x55_start;
				if (_0x55_delta_t > _0x55_max_t) {
					spin_lock(&i2c_turnaround.lock);
					_0x55_max_t = _0x55_delta_t;
					i2c_turnaround.kernel_time =
						local_clock();
					i2c_turnaround.max_delta_t =
						_0x55_delta_t;
					spin_unlock(&i2c_turnaround.lock);
				}
			}
		}
...
omap_i2c_probe(struct platform_device *pdev)
{
	...
	max_i2c_wtr_turnaround_0x55.kernel_time = 0;
	max_i2c_wtr_turnaround_0x55.max_delta_t = 0;
	spin_lock_init(&max_i2c_wtr_turnaround_0x55.lock);
